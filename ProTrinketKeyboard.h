...
#ifndef _PROTRINKETKEYBOARD_H_
#define _PROTRINKETKEYBOARD_H_
...
public:
  ...
  void
  poll(); // this (or "press" something) must be called at least once every 10ms
  char isConnected(); // checks if USB is connected, 0 if not connected

  // presses up to 6 keys, and modifiers (modifiers are keys like shift, CTRL,
  // etc)
